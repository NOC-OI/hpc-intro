...
The Amdahl code has one dependency: __mpi4py__. This is available on {{ site.remote.name }} via
the mpi4py module.

```
module load mpi4py
```
If it wasn't already installed on the cluster then we could use `pip` to
collect mpi4py from the Internet and install it for you. This can be quite
a slow process as it will compile several MPI libraries for you, especially
when a module is already built and configured to do it for us.
